...
client.on("ready",()=>{
	console.log("Asreaper - Youtube => OWO PARA KASMA BOTU AKTIF");
});
	...
	if(message.channel.id==kanal){
		if(message.author.id=="408785106942164992"){ // BURAYI ELLEMEYIN
			if(message.content.includes("Beep Boop") || message.content.includes("Please DM me") || message.content.includes("Are you a real human?")){
				db.set("owodg","1");
				message.channel.send("Can Fidan - Youtube owo para kasma sistemi durduruldu.");
			};
		};
	};
...
client.login("Token"); // HESAP TOKENI BURAYA